...
def bootstrap_mAP(df_precision_recall, n_bootstraps=5000,
    ...
    prefiscion_recall_tot dataframe'''
    ...
    for i in range(n_bootstraps):
        ...
        df_scores_boot = construct_df_scores_from_precision_recall_df(df_filt,
                                                                      outfile='', iou_thresh=iou_thresh,
                                                                      verbose=super_verbose)
        # print ("df_scores_boot)
        # compute f1, map for each category
        for cat in categories:
            mAP_boot, _, f1_boot, _ = simrdwn_eval.compute_map(df_scores_boot,
                                                               f1_thresh=f1_thresh,
                                                               category=cat,
                                                               verbose=super_verbose)
