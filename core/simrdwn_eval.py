...
# import shapefile parsing
import parse_shapefile
...
#    ##############
#    # Get precision - recall for entire class, old version
...
#        print ("\n", i, "Category:", category
#
#        ## keep only rows of the given category
#        gdf_truth_filt = gdf_truth[(gdf_truth['Category'] == category)]
#        df_prop_filt = df_prop[(df_prop['Category'] == category)]
#
#        score_list = []
#        vals_list = []
#        f1_list = []
...
#        for j,detect_thresh in enumerate(args.detect_threshes):
#            print i, j, "detect_thresh:", detect_thresh
#
#            # evaluate each image
#            n_true_pos_tot, n_false_pos_tot, n_false_neg_tot = 0, 0, 0
#            for k,im_root in enumerate(im_roots):
#                print i, j, k, "image_root:", im_root
#                count += 1
#                if show_plots:
#                    plot_file = os.path.join(args.out_dir, im_root \
#                                             + '_ground_truth_im_'+ category \
#                                             + '_' + str(detect_thresh) \
#                                             + '.png')
#                else:
#                    plot_file = ''
#
#                log, vals = eval_f1(gdf_truth_filt, df_prop_filt, im_root, category,
#                        detect_thresh=detect_thresh,
#                        iou_thresh=args.iou_thresh,
#                        nms_overlap=args.nms_overlap, plot_file=plot_file,
...
#
#            if (precision > 0) and (recall > 0):
#                score_list.append([detect_thresh, f1, precision, recall])
#                f1_list.append([n_ground_truth_boxes, f1])
#
#        score_list = np.array(score_list)
#        precision, recall = score_list[:,2], score_list[:,3]
#        f1 = score_list[:,1]
#
#        # get weighted mean f1
#        f1_arr = np.array(f1_list)
...
#        print ("Weighted STD F1:", std_f1
#
#        plot_precision_recall(precision, recall, outfile='', dpi=300)
...
def main():
    ...
    #df_plot_file = os.path.join(args.out_dir,  '_map.png')
    df_plot_file = os.path.join(
        args.out_dir,  '_map' + '_' + str(args.iou_thresh).replace('.', 'p') + '.png')
    plot_precision_recall_multi(df_scores_tot, outfile=df_plot_file,
                                figsize=(6, 6),
