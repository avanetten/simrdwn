...
scp -r /Users/avanetten/Documents/cosmiq/simrdwn/core 10.123.1.20:/raid/local/src/simrdwn/

Resize test images, may be necessaary if training resolution differs from native test resolution
...
def resize_dir(input_dir, output_dir, resize_factor=2, compression_level=3):
    ...
    for i,im_name in enumerate(im_list):
        ...
        im_out = cv2.resize(im, (int(width/resize_factor), int(height/resize_factor)),
        ...
        # write file
        if not use_skimage:
            cv2.imwrite(out_path, im_out, [cv2.IMWRITE_PNG_COMPRESSION, compression_level])
        else:
...
def main():
    ...
    parser.add_argument('--input_dir', type=str, default='/raid/data/xview/SuperRes/stitch_explore/Blurred_1Sigma_2x_tile',
                        help="images location")
    parser.add_argument('--output_dir', type=str, default='/raid/data/xview/SuperRes/stitch_explore/Blurred_1Sigma_2x_stitch',
    ...
    parser.add_argument('--compression_level', type=int, default=3,
                        help="compression level of image (0-9, 9 being most compressed and 3 is default")
